...
var validatorFactory = require("./validation/validatorFactory");
var requiredWhen = require("./validation/requiredWhen");
...
function validate(template) {
    ...
    for (var fieldName in template.data) {
        ...
        var vr = validateOne(field, template);
        if (!vr.valid) {
            errors.push(model.fieldError(fieldName, vr.errors));
        }
    }
...
function validateOne(field, template) {
    var errors = [];
    var required = requiredWhen.isRequired(field, template);

    validatorFactory.validate(required, field, errors);
    return model.validationResult(errors.length == 0, errors);
}
